...
void Shader::Texture(unsigned char* image,int TextureSize, int numComponents) {
	
	GLuint textures[1]; // define buffer of  textures pointer
	glGenTextures(1, textures);//generate  specific texture

	if (image == nullptr)
		std::cout << "Failed to load texture0" << std::endl;

	glBindTexture(GL_TEXTURE_2D, textures[0]);// binding to specific texture
	/*proccess image data*/
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, TextureSize,TextureSize,0, GL_RGB, GL_UNSIGNED_BYTE,image);
	/*replicate image on desired painted object if it doesnt fit according to s and t axes*/
